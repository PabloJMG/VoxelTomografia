...
void *propagacion(FILE  *fui, int *resul, int N, int NumPi, double bet, int con, int dix, int diy, double deltaPas, double deltX, double deltY, int factor, double anchura, double angma);
...
{
int i, j,  cont,  hit, no_hit, dimx, dimy, num_pix, factordim, NumAdq, N, NumPix, Niter, pixx, pixy, rc;
int array_inp[3];
double  d, deltaX, deltaY, deltaP, betarnd, num_rand, mu, anchura_malla, delta, deltadis, deltaang, beta, deltaPaso, angmax;
...
deltaPaso=0.0001; /*Intervalo de propagación del rayo en cada iteración*/
delta=d; /*Distancia de la fuente al objeto*/

deltaP=anchura_malla/NumPix; /*Tamaño de cada detector*/
...
trataobj(fu1, factordim,  dimx, dimy, factordim*dimx/8, factordim*dimy/8); /*Crea objeto y lo guarda en un archivo de texto*/
...
for(i=0; i<NTHREADS; i++)
{
	...
	thread_arg[i]=i;
rc=pthread_create(&threads[i], NULL, propagacion(fu1, resultados, array_inp[1], array_inp[2], beta, cont, dimx, dimy, deltaPaso, deltaX, deltaY, factordim, anchura_malla, angmax),(void *) &thread_arg[i]);
...
printf("Final del ángulo %lf\n", beta);
	 ...
	 //~ fclose(fu2);
	 fu2=apertura_archivo(cont); /*Crea un archivo para cada ángulo*/

	guarda_resultados(resultados, NumPix, fu2); /*Guarda los rayos detectados por cada pixel*/
	fclose(fu2);
	free(resultados);
}
//~ printf( "\nHan llegado = %d \nNo han llegado %d de %lu rayos\n", hit, no_hit, N*NumAdq);
	 ...
	 for (i=0; i<NTHREADS; ++i) /*Este for dónde va ¿?¿?*/
	 {
...
void *propagacion(FILE  *fui, int *resul, int N, int NumPi, double bet, int con, int dix, int diy, double deltaPas, double deltX, double deltY, int factor, double anchura, double angma) /*N=argv[1]*/
{
	double num_rand, betarnd, deltadis, deltaP, Niter, delta, d, mu;
	double pos0[2], pos1[2];
	int i, j, pixx, pixy, numpix, num_pix;
	int *pix=malloc(2*sizeof(int));
...
if(pixx != *(pix) || pixy != *(pix+1)) /*Esto es un OR */
	...
	num_rand=rand()/((double)RAND_MAX +1);
       ...
       /*printf("el angulo es %d la N es = %dla j es = %d y el pixel %d %d, cuya mu es %lf\n", cont, i,  j, *pix, *(pix+1), mu);*/
		...
		if(num_rand > exp(-mu*deltX)) /*Pasa o no el rayo*/
				{	
