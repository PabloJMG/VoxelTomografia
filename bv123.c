...
void guarda_matriz2(double *resu, int dimxx, int dimyy, FILE *fgu);
...
int dimx, dimy,  i, cont,  j, q,  numpixx2, numpixy2, numpixx, numpixy, pasx, factordim, NumAng, conta, Num_Pix, dimtotal; /*Variables n, d no usadas*/
...
numpixx=(int) *numpix;
numpixy = (int) *(numpix+1);

if(numpixx2!=numpixx || numpixy2!=numpixy)
...
*(matriz_resul+numpixy*dimx*factordim+numpixx)+=(1-numpix[2]) + (1-numpix[3]);
*(matriz_resul+numpixy*dimx*factordim+numpixx+1)+=numpix[2] + (1-numpix[3]);
*(matriz_resul+(numpixy+1)*dimx*factordim+numpixx)+=(1-numpix[2]) + numpix[3];
*(matriz_resul+(numpixy+1)*dimx*factordim+numpixx+1)+=numpix[2] + numpix[3];
conta+=1;	
...
#ifdef WEIGHT_SUM
guarda_matriz2(matriz_resul,factordim*dimx, factordim*dimy, fout);
#endif
free(matriz_resul);
free(datainp);
//~ free(numpix);
...
fprintf(fgu,"%lf   ", *(resu+con1*dimxx+con2));
}
fprintf(fgu, "\n");
