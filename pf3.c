...
angmax=anchura_malla/(2*(anchura_malla+d)); 
	...
	//~ N.par=array_inp[1];
	//~ NumPix.par=array_inp[2];
	//~ resultados.par=malloc(NumPix.par[i]*sizeof(int));
	//~ beta.par=deltaang*i;
	//~ contador.par[i]=i;
	//~ dix.par=dimx; /*De momento poner esto*/
	//~ diy.par=dimy;
	//~ deltaPaso.par=0.0001;
	//~ deltaX.par=0.01;
	//~ deltaY.par=0.01;
	//~ factordim.par=2;
	//~ anchura.par=anchura_malla;
	//~ angmax.par=angmax;
...
fin=clock();
printf("El tiempo de ejecución es %lf", (double)(fin-comienzo)/CLOCKS_PER_SEC);
...
void *propagacion(void *threadarg) /*N=argv[1]*/
{
	...
	int i, j, pixx, pixy, num_pix, NRayos, cont;
	int *pix=malloc(2*sizeof(int));
	struct param *mispar;
	...
	int *resultados =malloc(NumPi*sizeof(int));
