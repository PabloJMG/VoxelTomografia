...
float beta, cosbeta, factora, sinbeta, deltaang, lambda, deltax, deltay, deltaf, posx, posy, posx2, posy2, val, diferencia, cant, sumpix;
double angulos[10];
FILE *finp;
 ...
 sumpix=0;
	 for(cont=0; cont<NumAng; cont++)
	 {
		for(cont2=0; cont2<dimx; cont2++)
		{
			for(cont3=0; cont3<dimy; cont3++)
			{
				...
				diferencia+=(*(b+cont*NumPix+cont4)-cant)*(*(a+cont*NumPix*dimtotal+cont4*dimtotal+cont2*dimy+cont3))/(*(betar+cont*NumPix+cont4)); /* Aquí mirar  */
					//~ diferencia+=(*(b+cont*NumPix+cont4)-cant)/(*(betar+cont*NumPix+cont4));
			//~ printf("diferencia=%lf\n", diferencia);
			//~ printf("b = %lf, cantfinal=%lf, cont =%d, cont4=%d\n ", *(b+cont*NumPix+cont4), cant, cont, cont4);
			//~ getchar();
		...
		*(x+cont2*dimy+cont3)+=lambda*diferencia/(*(gamma+cont*dimtotal+cont2*dimy +cont3));
		//~ printf("update del %d, %d es %lf\n", cont2, cont3, lambda*diferencia);
		deltaf+=diferencia;
		//~ printf("x=%lf, xdespues=%lf, difer=%lf, gamma=%lf numang =%d, dimx=%d dimy=%d numpix=%d, i=%d\n", *(x+cont2*dimy+cont3), *(xdespues+cont2*dimy+cont3), diferencia, *(gamma+cont*dimtotal+cont2*dimy+cont3), cont, cont2, cont3, cont4, i);
		//~ getchar();
...
//~ {
	//~ for(cont3=0; cont3<dimy; cont3++)
	//~ {
		//~ printf("x=%lf, xdespues=%lf\n", *(x+cont2*dimy+cont3), *(xdespues+cont2*dimy+cont3));
		//~ *(x+cont2*dimy+cont3)=*(xdespues+cont2*dimy+cont3);
...
fprintf(pipeplot, "splot '/home/pablo/Resultados_C/FP_BP_Octubre/Resultados_SART.txt' matrix\n");
sleep(5);
...
void *carga_archivo(int NumAng, int NumPix, float *b)
{
 ...
 for(cont=0; cont<NumAng; cont++) /*  Cargar b (esto está mal)*/
 {
	...
	fscanf(finp, "%d", &valu);
	printf("val=%d\n", valu);
	*(b+cont*NumPix+cont2)=valu; /*Esto aquí lo he cambiado*/
